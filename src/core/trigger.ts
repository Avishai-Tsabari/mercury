...
export function loadTriggerConfig(
  ...
  const mediaInGroups =
    db.getSpaceConfig(spaceId, "trigger.media_in_groups") === "true";
...
export function matchTrigger(
  ...
  if (!trimmed) {
    if (hasAttachments) {
      if (isDM) return { matched: true, prompt: "" };
      if (config.match === "always") return { matched: true, prompt: "" };
      if (config.mediaInGroups) return { matched: true, prompt: "" };
    }
...
function matchSinglePattern(
  ...
  switch (config.match) {
    ...
    case "mention": {
      ...
      // Match at start-of-string or after whitespace, then end the trigger at
      // whitespace, common punctuation (e.g. "@Mercury, hi"), or EOS — but not
      // another letter/digit (avoids "Pi" inside "Pixel").
