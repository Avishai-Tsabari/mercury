...
describe("matchTrigger — mention mode", () => {
  ...
  test("matches @Pi followed by comma (group natural typing)", () => {
    const r = matchTrigger("@Pi, what time is it?", cfg(), false);
    expect(r.matched).toBe(true);
    expect(r.prompt).toBe("what time is it?");
  });

  test("matches Mercury, … for bare-name pattern", () => {
    const r = matchTrigger(
      "Mercury, summarize the thread",
      cfg({ patterns: ["@Mercury", "Mercury"] }),
      false,
    );
    expect(r.matched).toBe(true);
    expect(r.prompt).toBe("summarize the thread");
  });
...
describe("matchTrigger — always mode", () => {
  ...
  test("empty text matches in always mode when attachments present", () => {
    const r = matchTrigger("", cfg({ match: "always" }), false, true);
    expect(r.matched).toBe(true);
    expect(r.prompt).toBe("");
  });
});
...
describe("matchTrigger — DM behavior", () => {
  ...
  test("DM with attachment only (no text) matches", () => {
    const r = matchTrigger("", cfg(), true, true);
    expect(r.matched).toBe(true);
    expect(r.prompt).toBe("");
  });

  test("group attachment-only matches when mediaInGroups is enabled", () => {
    const r = matchTrigger("", cfg({ mediaInGroups: true }), false, true);
    expect(r.matched).toBe(true);
    expect(r.prompt).toBe("");
  });
});
