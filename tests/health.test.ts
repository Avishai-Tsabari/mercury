...
describe("Health response structure", () => {
  test("health response has required fields", () => {
    ...
    expect(healthResponse.extImage).toBeNull();
  });
});

describe("extImage field in health response", () => {
  function deriveExtImage(currentImage: string): string | null {
    return currentImage.startsWith("mercury-agent-ext-") ? currentImage : null;
  }

  test("returns ext image tag when container runner uses a derived ext image", () => {
    const image = "mercury-agent-ext-95688bfb:08913c4ae26a";
    expect(deriveExtImage(image)).toBe(image);
  });

  test("returns null when container runner uses the base host image", () => {
    expect(deriveExtImage("ghcr.io/owner/mercury-host:0.9.9")).toBeNull();
  });

  test("returns null when build is in progress (currentImage is base image)", () => {
    expect(deriveExtImage("mercury-agent:latest")).toBeNull();
  });

  test("returns null for an empty image string", () => {
    expect(deriveExtImage("")).toBeNull();
  });
});
