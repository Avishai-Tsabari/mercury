...
describe("HookDispatcher — before_container", () => {
  ...
  it("collects promptAppend from single handler", async () => {
    ...
    expect(result?.promptAppend).toBe("[Voice] hello");
  });

  it("concatenates promptAppend from multiple handlers", async () => {
    writeExt(
      "append-a",
      `export default function(m) {
				m.on("before_container", async () => ({ promptAppend: "Part A" }));
			}`,
    );
    writeExt(
      "append-b",
      `export default function(m) {
				m.on("before_container", async () => ({ promptAppend: "Part B" }));
			}`,
    );
    await registry.loadAll(extDir, db, log);
    dispatcher = new HookDispatcher(registry, log);
    ...
    expect(result?.promptAppend).toContain("Part A");
    expect(result?.promptAppend).toContain("Part B");
  });

  it("merges systemPrompt and promptAppend together", async () => {
    writeExt(
      "both-ext",
      `export default function(m) {
				m.on("before_container", async () => ({
					systemPrompt: "SYS",
					promptAppend: "USER_APPEND",
				}));
			}`,
    );
    await registry.loadAll(extDir, db, log);
    ...
    expect(result?.promptAppend).toBe("USER_APPEND");
  });
});
...
describe("HookDispatcher — after_container", () => {
  ...
  it("merges files from a handler", async () => {
    writeExt(
      "with-files",
      `export default function(m) {
				m.on("after_container", async () => ({
					files: [
						{
							path: "/tmp/a.txt",
							filename: "a.txt",
